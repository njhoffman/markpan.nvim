...
local function ts_query_get_ranges(parser, root, query_string, capture_name, prune_same_line)
  capture_name = capture_name == nil and "v" or capture_name
  prune_same_line = prune_same_line == nil and true or false
  local line_ranges = {}
  local query = vim.treesitter.query.parse(parser:lang(), query_string)
  for _, matches, metadata in query:iter_matches(root, 0) do
    for id_, match in ipairs(matches) do
      local name = query.captures[id_]
      if name == capture_name then
        local start, start_col, _, stop, stop_col, _ = unpack(vim.treesitter.get_range(match))
        if not (prune_same_line and start == stop) then
          table.insert(line_ranges, { start, stop, start_col, stop_col, match })
        end
      end
    end
  end
...
local function get_header_ranges(parser, root)
  local header_ranges =
    ts_query_get_ranges(parser, root, string.format("(line . [(h1) (h2) (h3)]) @v", type))

  -- We combine header_ranges & tag ranges only when they overlap
  local tag_ranges =
    ts_query_get_ranges(parser, root, '((line . [(h1) (h2) (h3)]) . (line . (tag)+ . "\\n"?)* @v)')
  vim.list_extend(
    tag_ranges,
    ts_query_get_ranges(parser, root, '((line . (tag)+) . (line . (tag)+ . "\\n"?) @v)')
  )
  ...
  for _, range in ipairs(ranges) do
    ...
    if not lines_are_empty then
      ...
      table.sort(indents, function(a, b)
        return a:gsub(" ", "s"):gsub("\t", " "):gsub("s", " ")
          > b:gsub(" ", "s"):gsub("\t", " "):gsub("s", " ")
      end)
...
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function()
    ...
    for _, range in ipairs(fn_desc) do
      local start, stop, start_col, stop_col, node = unpack(range)
      local text = vim.treesitter.get_node_text(node, 0, {}):gsub(": ~", "")
      local icon = icon_map[text] or ""
      vim.api.nvim_buf_set_extmark(0, ns, start, stop_col - 1, {
        virt_text = { { icon, "@markup.heading.4.vimdoc" } },
        virt_text_pos = "inline",
      })
    end

    local tag_ranges = ts_query_get_ranges(parser, root, "(tag) @v", "v", false)
    for _, range in ipairs(tag_ranges) do
      local start, stop, start_col, stop_col, node = unpack(range)
      vim.api.nvim_buf_set_extmark(0, ns, start, start_col, {
        virt_text = { { " ", "@label.vimdoc" } },
        virt_text_pos = "inline",
      })
    end

    local argument_ranges = ts_query_get_ranges(parser, root, "(argument ((word) @v))", "v", false)
    for _, range in ipairs(argument_ranges) do
      local start, stop, start_col, stop_col, node = unpack(range)
      vim.api.nvim_buf_set_extmark(0, ns, start, start_col, {
        virt_text = { { " ", "@variable.parameter.vimdoc" } },
        virt_text_pos = "inline",
      })
    end

    local taglink_ranges = ts_query_get_ranges(parser, root, "(taglink ((word) @v))", "v", false)
    for _, range in ipairs(taglink_ranges) do
      local start, stop, start_col, stop_col, node = unpack(range)
      vim.api.nvim_buf_set_extmark(0, ns, start, start_col, {
        virt_text = { { " ", "@markup.link.vimdoc" } },
        virt_text_pos = "inline",
      })
    end
  end,
