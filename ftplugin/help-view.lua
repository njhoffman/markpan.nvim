...
local parser_installed = function(parser_name)
  return (ts_available and treesitter_parsers.has_parser(parser_name))
    or vim.treesitter.query.get(parser_name, "highlights")
end
...
if
  vim.islist(helpview.configuration.buf_ignore)
  and vim.list_contains(helpview.configuration.buf_ignore, vim.bo.buftype)
then
  return
end

local help_augroup =
  vim.api.nvim_create_augroup("helpview_buf_" .. vim.api.nvim_get_current_buf(), { clear = true })
  ...
  callback = function(event)
    ...
    if helpview.state.enable == false or helpview.state.buf_states[buffer] == false then
      for _, window in ipairs(windows) do
        if
          helpview.configuration.options
          and helpview.configuration.options.on_disable
          and pcall(helpview.configuration.options.on_disable, window, buffer)
        then
      ...
      for _, window in ipairs(windows) do
        if
          helpview.configuration.options
          and helpview.configuration.options.on_enable
          and pcall(helpview.configuration.options.on_enable, window, buffer)
        then
    ...
    helpview.renderer.render(
      buffer,
      parsed_content,
      helpview.configuration,
      helpview.get_buffer_info(buffer)
    )
  end,
})
...
vim.api.nvim_create_autocmd({ "ModeChanged", "TextChanged", "WinResized" }, {
  ...
  callback = function(event)
    ...
    if
      event.event ~= "WinResized" and modifiable == false
      or lines > (helpview.configuration.max_lines or 1000)
    then
      local cursor = vim.api.nvim_win_get_cursor(0)

      if
        vim.islist(helpview.configuration.modes)
        and vim.list_contains(helpview.configuration.modes, mode)
      then
        helpview.renderer.partial_clear(
          buffer,
          cursor[1] - (helpview.configuration.render_lines or window_lines),
          cursor[1] + (helpview.configuration.render_lines or window_lines)
        )
        helpview.renderer.partial_render(
          buffer,
          cursor[1] - (helpview.configuration.render_lines or window_lines),
          cursor[1] + (helpview.configuration.render_lines or window_lines),
          helpview.configuration,
